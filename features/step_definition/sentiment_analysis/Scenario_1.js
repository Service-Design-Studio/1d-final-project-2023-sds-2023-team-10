...
When(
  "she clicks on the 'Message' icon",
  { timeout: 60 * 1000 },
  async function () {
...
When("selects her usual counsellor", { timeout: 60 * 1000 }, async function () {
  ...
  await page.click(
    "#__next > div > div > div.py-5 > div > ul > li:nth-child(2)"
  );
});
...
When("clicks the Send button", async function () {
  ...
  // await page.waitForSelector(
  //   "#__next > div > div > div.py-5 > div > div > div > div.css-sdxdhq > button"
  // );
...
When(
  "she types I'm worried about my health",
  { timeout: 60 * 1000 },
  async function () {
    // Wait for the text box to appear on the page
    await page.waitForSelector("input.chakra-input.css-1fpoko8");
...
Then("the admin sees a graph", { timeout: 60 * 1000 }, async function () {
  await page.goto(`${adminLocalTestUrl}/login`, {
    waitUntil: "networkidle0",
    timeout: 60000,
  });
  ...
  const messageUrl = `${adminLocalTestUrl}/chat`; // Replace with the desired URL
  await page.goto(messageUrl, { waitUntil: "networkidle0" });
  // const chatSelector =
  //   "#__next > section > main > div > div:nth-child(1) > div > div > div > div > div > li:nth-child(57)"; // replace with your button selector
  const chatSelector = '[data-testid="chatroom-58"]';
  ...
  await page.waitForSelector('[data-testid="graph"]', { timeout: 5000 }); // waits for the graph to be displayed on the page
